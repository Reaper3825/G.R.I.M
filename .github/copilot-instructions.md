...
- **Embedding init = Normal(0,1), NOT Xavier** (Issue #163): Xavier gives embedding RMS ≈ 0.014 for V=10000+, D=768. After 12 pre-norm encoder layers, sublayer noise accumulates to RMS ≈ 0.88, giving SNR ≈ 0.016 — the embedding signal is 60× weaker than noise. This causes violent norm divergence and rho spike at step ~200, poisoning weights with correlated gradient signal. Normal(0,1) gives embedding RMS ≈ 1.0, bringing SNR to ~1.14. Do NOT scale embeddings by `sqrt(d_model)` in the forward pass — ALiBi/RoPE inject position info inside attention.
- **γ_final init = 1/sqrt(d_model)** (Issue #163): Compensates for N(0,1) embedding init with tied weights. With embedding RMS ≈ 1.0, `gamma_init = 1/sqrt(768) ≈ 0.036` keeps initial logit std ≈ 1.0 (healthy softmax). γ_final learns with `lr_mult=0.1` and `wd_mult=1.0`.
- **γ_final (final RMSNorm) MUST have weight decay AND slow LR**: Without weight decay, γ_final acts as an unregulated logit temperature and grows unbounded (1.0→3.0), causing overconfident predictions → mode collapse. Registered as `ParamGroupType::RMSNORM` with `wd_mult=1.0` (NOT 0.0) AND `lr_mult=0.1` (10× slower learning rate). Weight decay provides long-term restoring force; `lr_mult=0.1` prevents the initial spike (γ_final has a monotonic "scale up" gradient bias because scaling logits reduces CE faster than learning representations). Per-layer gammas (γ₁, γ₂) remain `RMSNORM` with no decay and `lr_mult=1.0` — they get mixed gradient signals through encoder nonlinearities that naturally constrain growth.
