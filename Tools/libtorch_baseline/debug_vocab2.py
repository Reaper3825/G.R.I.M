...
with open(vocab_path, 'rb') as f:
    ...
    print("First 20 pieces (showing hex for stored_id):")
    
    for i in range(min(20, config_vocab)):
        piece_len = struct.unpack('<I', f.read(4))[0]
        text_bytes = f.read(piece_len)
        text = text_bytes.decode('utf-8', errors='replace')
        score = struct.unpack('<f', f.read(4))[0]
        stored_id = struct.unpack('<I', f.read(4))[0]
        
        computed_id = 512 + i
        print(f'  {i}: text="{text}" (bytes={text_bytes.hex()}) stored_id={stored_id} (0x{stored_id:04x}) computed_id={computed_id}')

print("\nByte value of 'e':", ord('e'))
print("Byte value of 'C':", ord('C'))
