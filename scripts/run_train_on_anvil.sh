...
#   --partition P  SLURM partition (default: gpu). Use gpu-debug for short test runs (often shorter queue).
...
#   --time T      Wall clock limit (default: 4:00:00). Shorter (e.g. 2:00:00) can get backfilled sooner.
...
# Override Anvil gpu partition DefMemPerGPU=120000 so we request 64G and schedule faster.
