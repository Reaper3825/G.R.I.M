...
def load_embedding(speaker_id, device="cpu"):
    ...
    try:
        if np is not None:
            ...
            gpt_cond = torch.from_numpy(data['gpt_cond_latent']).to(device)
            spk_emb = torch.from_numpy(data['speaker_embedding']).to(device)
...
def persistent_loop(model_name, speaker, use_gpu=True):
    ...
    try:
        ...
        # ============================================
        # ✅ OPTIMIZATION: GPU Settings + Model Warm-up
        # ============================================
        if device == "cuda":
            ...
            log("[Coqui XTTS] GPU optimizations enabled (TF32, cudnn benchmark)")

        # Warm up model with dummy synthesis
        log("[Coqui XTTS] Starting model warm-up...")
        try:
            dummy_gpt, dummy_spk = load_embedding("default", device)
            ...
            if cmd == "exit":
                ...
                try:
                    ...
                    if is_xtts:
                        ...
                        if use_embedding:
                            gpt_cond_latent, speaker_embedding = load_embedding(spk, device)
