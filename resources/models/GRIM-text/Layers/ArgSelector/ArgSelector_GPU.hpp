...
//
//  Given the encoder hidden states and externally materialized candidate keys,
//  produce a selection-logit matrix
...
//  query projection train). The caller owns the gradient policy for `keys`.
//  Scores are produced with autograd::matmul so backward reuses the tested
//  matmul GradFns; the row-window mask is an additive constant.
//======================================================//
...
Tensor argSelectorForward(
    ...
    const Tensor& keys,            // [num_pool_atoms, d_model], caller-owned
