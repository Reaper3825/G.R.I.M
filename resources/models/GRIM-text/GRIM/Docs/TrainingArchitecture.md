...
`GRMTDataLoader` owns only raw `.grmt` row deserialization into `TrainingSequence` objects. It must not expose stable `seq_id` sample accessors or long-lived row views, because Phase 1 still has to split train/val data, inject boundaries, apply sliding windows, and filter invalid rows. `LoadTrainingData()` passes `TokenizerHP`, `DataLoadingHP`, and `CapacityHP` directly into this startup data path; do not add local wrapper structs that mirror those grouping fields. Training `seq_ids` are authored only after `applySlidingWindows()` when `TrainingData.cu` builds `train_views` / `val_views`; `Batching_GPU.cu` schedules those post-window view indices, and `BatchPayload.cu` consumes them.
