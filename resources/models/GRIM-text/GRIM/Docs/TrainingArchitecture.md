...
The **Optimizer Window** is the only Phase 2 boundary where a completed accumulation window becomes updated weights. Configured optimizer stepping lives in `Shared/Optimizers/OptimizerUpdate_GPU.{hpp,cu}` behind `launchOptimizerUpdate()`. Phase 2 must not branch on `optimizer_kind` or hand-wire AdamW/RAdamW launch arguments; it passes the grouped `OptimizerUpdateHP`, learning rate, step, stream, and registered parameter groups after the accumulation window is full. AdamW/RAdamW-specific kernel launches and embedding-freeze skip semantics belong to the optimizer dispatcher, while each concrete optimizer file owns only its own kernel/step implementation. See [Optimizer.md](Optimizer.md) for the full boundary policy.
