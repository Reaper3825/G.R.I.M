...
`RhoDiagnostic` now emits two boundary-specific equation logs from `processBatch()`: `RHO_BUILDUP_EQUATION_PRE_BACKWARD` after forward/loss assembly and before `executeAutogradBackward()`, and `RHO_BUILDUP_EQUATION` after backward returns. The pre-backward emission is log-only and must not overwrite telemetry `last_obs[]`; the post-backward emission remains the telemetry-writing path.

`processBatch()` also emits explicit chronology breadcrumbs around the shared training graph: `EXPLICIT_TRAINING_FORWARD_COMPLETE` immediately after `Forward::executeModelForward(...)` returns, and `EXPLICIT_TRAINING_BACKWARD_COMPLETE` immediately after `executeAutogradBackward(...)` returns. These exist specifically to disambiguate the pre-forward `ForwardPass: BATCH_TARGET_DIST` payload summary from the real forward/backward boundaries in `training_<session>.log`.
...
`training/Diagnostics/DiagnosticInference.cu` runs the training-time sample generator only when `training.config.inference_diagnostic_enabled=true` and `optimizer_step % training.config.inference_diagnostic_interval == 0`. The interval must be positive when the diagnostic is enabled. `GRIM_SAMPLE_INTERVAL` is not part of the cadence contract; orchestration passes the config-owned toggle and interval through the diagnostic call signature.
...
`Layers/FlashAttention/AttentionBreadthDiagnostic.cu` owns the layer-recorder entry tagged `[ATTN_BREADTH]` in the attention log file `logs/Attention/<layer>.log` (today the FlashAttention equation sampler passes `layer_idx=-1`, so the output lands in `logs/Attention/-1.log`). It is **not** a BatchLogTape / `training_<session>.log` entry and it is **not** EquationSink output. The standalone hook consumes a **full causal score row** `score[t,0..t]` plus the matching exact `lse(t)` and reconstructs
$$
\alpha_{t,u} = \exp(\mathrm{score}_{t,u} - \mathrm{lse}_t)
$$
before reporting entropy, normalized entropy, effective support $\exp(H)$, participation ratio $1 / \sum_u \alpha_{t,u}^2$, and `alpha_max`. The emitted entry is a compact layer-recorder line so it fits the recorder message budget; `normalized_entropy` and `uniform_fraction` are also copied into the entry scalars as `primary` and `secondary`. This diagnostic is specifically for the “is attention behaving like broad prefix averaging / reducing token distinctness?” question. Do **not** feed sampled or truncated rows into it — a partial row undercounts mass and invalidates the breadth measurement.
...
Clip component metrics map only registered `ParamGroupType` values. Telemetry streams 14–15 are reserved because no execution-block parameter-group category exists; do not publish permanent-zero execution gradient measurements or add unregistered ScratchBlock/execution accumulators to `ClipMetrics`.

Clip-result diagnostics validate the `ClipResult` contract directly: pre/post global RMS values must be finite and non-negative, post RMS must not exceed pre RMS, clipped results must actually reduce RMS, and unclipped results must preserve RMS within tolerance. Group-count checks compare `ClipMetrics.groups_processed` against `ClipResult.measured_group_count`, not against the registry-owned parameter-group vector size, so future filtered/fused clipping topologies remain valid.
...
## TelemetryLattice
Hierarchical streaming statistics use 8 levels. Stream 25 (`eb_loss_frac`) is the execution-loss fraction `execution_loss / total_loss`. Stream 26 is reserved; historical CSVs may still label it `mtp_loss_frac` or `sb_atom_embed_rms`. Stream 38 (`rho_raw_rms_spread`) is the canonical hidden-state health signal — see [LMHead.md](LMHead.md). Streams 39–44 track h↔W LM-head alignment, streams 45–46 track unigram-frequency-direction alignment, stream 47 tracks the raw `lm_head_w_rms_rms` term used by the logit-scale equation, streams 48–54 publish the compact init-time tied-embedding / optimizer-group invariant mirror directly into `telemetry_<session>.csv`, streams 55–57 track final-layer rho signed/centered/mean-vector diagnostics, and stream 60 (`optimizer_iteration`) mirrors `input.optimizer_step + 1`.
...
## Atom-token display
Diagnostics may call `UniByte::decode({tid})` on atom-boundary IDs because each ID renders as its literal typed tag (`<INT>`, `</INT>`, `<FLOAT>`, or `</FLOAT>`). Full values are reconstructed from the ordinary model-visible tokens between those boundaries, not from `atom_entry_ids` or `AtomTable`. Aggregate displays such as Rho top-token logs should preserve the opening/closing distinction.

`AtomIdentifierDiagnostic` follows the separate atom-identification task ABI, not the tokenizer's twelve boundary-token ABI. It reports the six typed OPEN decisions and one type-free EXIT decision. EXIT has no independent atom type to diagnose: inference must render its close tag from the opening type persisted by the active request-local span state.
