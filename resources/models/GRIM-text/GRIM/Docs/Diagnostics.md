...
Hidden-state diagnostics must observe the live LM-head input tensor inside the active forward/autograd boundary. Prefer `AutogradIntermediates::centered_encoder_output` when populated, otherwise `AutogradIntermediates::encoder_output_tensor`. Do not recreate durable TrainingState snapshots such as `cached_encoder_output`; they turn current-forward observations into stale global mailboxes. Runtime inference consumers copy logits from `AutogradIntermediates::logits_tensor` before `AutogradIntermediates::clear()`; hidden-state observations must come from live intermediates in the same boundary.

`LMHeadLayer::forward` itself must not write that diagnostics handle into runtime payload storage. It returns the live LM-input tensor (when it had to materialize one) and the top-level shared forward stores it in `ModelForwardOutputs` / `AutogradIntermediates` if the caller wants same-boundary observation.
...
Logit diagnostics must receive the live logits tensor explicitly while `AutogradStepScope` is still active. Do not add `TrainingState` logits caches or read logits through durable `cached_*` fields.
