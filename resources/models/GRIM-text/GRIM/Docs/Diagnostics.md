...
`AtomIdentifierDiagnostic` follows the separate atom-identification task ABI, not the tokenizer's ten boundary-token ABI. It reports the five typed OPEN decisions and one type-free EXIT decision. EXIT has no independent atom type to diagnose: inference must render its close tag from the opening type persisted by the active request-local span state.
