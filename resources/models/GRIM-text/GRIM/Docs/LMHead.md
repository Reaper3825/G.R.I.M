...
- Live Issue #132 order in `LMHeadLayer::forward`: `RMSNorm(h)` → optional `center_columns_by_sequence_lengths(...)` → optional `project_out_pc1(...)` → `center_rows(W_lm)` → `autograd::matmul(lm_input, W_eff, transpose_b=true)`. The row-centering stage is therefore after hidden column-centering, but it is applied to `W_lm` rather than destructively row-centering `h`.
