...
- `LMHeadLayer::forward` must also stay read-only over caller runtime sinks. If the head materializes a transformed LM input (`RMSNorm(h)`, causal-prefix-centered `h`, or `project_out_pc1(...)`), it returns that tape-local tensor to the caller as part of the forward result; `ModelForward` decides whether to stash it in `ModelForwardOutputs::centered_encoder_output` for same-boundary diagnostics. Do not reintroduce `Tensor& out_*` output refs that let the layer write directly into runtime payloads.
