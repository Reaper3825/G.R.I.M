...
| `DataLoadInputs` | Deleted local Phase1 data-load mirror; `LoadTrainingData()` consumes `TokenizerHP`, `DataLoadingHP`, and `CapacityHP` directly, while `DataInfo` remains only the durable GRMT/data summary fact stored on `TrainingContext`. |
