...
- `ManualVocabSource.hpp/.cpp` — authored JSON source parsing and piece 
...
| `[ATOM_TOKEN_OFFSET, ATOM_TOKEN_END)` | Typed atom opening/closing boundaries |
| `NEWLINE_TOKEN_ID` | Fixed canonical newline token (`LF`, `CR`, and `CRLF` input) |
...
- `ConceptBlockGrmtCompiler.cu` writes `UniByte::vocabSize()` into the `.grmt` header when it encodes training data.
...
- `Shared/GRMT/GrmtFormat.hpp` owns `.grmt` magic, header layout, version , and header write helpers. Consumers must call `readHeaderOrThrow()` / `readHeaderStatus()` / `writeHeaderOrThrow()` instead of open-coding magic/version reads.
...
- `Shared/TokenizerArtifacts/GrmtSequence.hpp` is the canonical GRMT row payload type: token IDs, supervision targets, atom side channels, and execution metadata. Move row-shape fields and row-level  methods there instead of burying them inside reader/writer headers.
- GRMT v23 persists top-level ConceptBlock `knowns` and `unknowns` as ordered entry token IDs plus half-open logical token spans. This metadata is row-aligned through sliding windows, `BatchPayload`, and model-forward views, and remains independent of `Goal`.
- GRMT v22 adds a per-row `SequenceLocalAtomTable` and an opening-only `token_local_atom_indices` channel. A local address is `(AtomType, local_index)`: the opening boundary token supplies the type, so no duplicate per-token type channel is serialized. This address space is independent of `AtomTable::id` / `atom_entry_ids` and is metadata, not a supervision target. Sliding-window construction rebuilds a compact local table for each final row and remaps only these local indices.
- `BatchPayload` carries that opening-only index channel and derives compact local-selection metadata once after final windowing. Queries are typed openings; candidate banks are segmented by batch row and atom type; each candidate exposes the content-token positions and closing position of its first complete occurrence. Runtime targets use `0 = NO_REFERENCE` and `local_index + 1 = reference`, so they require no additional GRMT field and never consult `atom_entry_ids`.
- A typed opening boundary remains the sole metadata-bearing position and must resolve independently to both its durable `AtomTable` entry and its sequence-local typed value. Ordinary content tokens and typed closing boundaries carry `kAtomEntryNone` and `kLocalAtomIndexNone`. `GrmtSequence::token_atom_aux_target_mask` remains runtime-only: sliding-window construction authors it after BOS/EOS insertion and final slicing, `BatchPayload` directly masks matching LM target rows, and GRMT writers reject attempts to serialize it.
...
- `Shared/TokenizerArtifacts/GrmtCorpusIO.hpp/.cu` is the single GRMT row I/O primitive. It owns RAII file open/close, temp-file cleanup, header writes, row serialization/deserialization, and fail-loud  for persisted row contents; the `GrmtSequence` type itself lives in `GrmtSequence.hpp`.
...
- `ConceptSupervision.cu` owns ConceptBlock field-policy projection;
  `SlidingWindow.cu` owns `BOS`/`EOS` insertion, window construction, and
  window-boundary target fixup.
...
- Fixed numeric IDs and `NEWLINE_TOKEN_ID` decode from layout-owned definitions; they are not learned pieces and are not serialized as learned-vocabulary records in `vocab.bin`.
...
- The independent non-causal atom-identification model does not learn the twelve tokenizer boundaries as twelve independent decisions. Its task-local gap ABI is `[OPEN_INT, OPEN_FLOAT, OPEN_STRING, OPEN_BOOL, OPEN_ENTITY, OPEN_TOOL, EXIT]`: OPEN persists the selected type in request-local decoder state, and EXIT renders the matching typed close token from that state. The model input is always `[BOS, exact UTF-8 byte tokens, EOS]`, including ENTITY surfaces and TOOL identifiers; unigram pieces are not used by this task. The task reuses a contiguous seven-row window of the existing full-vocabulary LM head, so it adds no task-specific tokenizer IDs, model parameters, GRMT fields, or durable span state. Keep `Tokenizer::ATOM_VOCAB_SIZE` at the full twelve-token tokenizer ABI; atom decision geometry is owned by `Shared/AtomInsertion/AtomInsertionDecisionLayout.hpp`.
...
- Horizontal ASCII spacing bytes are rewritten before unigram segmentation: space and tab become the shared SentencePiece `▁` marker. LF, CR, and CRLF are canonicalized to one fixed `NEWLINE_TOKEN_ID` (318), which decodes as LF; learned unigram IDs begin at 319. Raw line-ending byte fallback tokens never survive into the runtime segmentation stream.
...
- CUDA Viterbi kernels report logical  failures through the explicit `error_code` pointer and named `kUnigramViterbiCuda*` status constants. Do not use device `assert()` for correctness checks; callers/tests must synchronize, copy the status word, and fail if it is not `kUnigramViterbiCudaOk`.
...
- Vocab-character  must use `utf8DecodeAt()` for the single-codepoint decode. Do not hand-decode multi-byte candidates; continuation-byte checks, overlong rejection, surrogate rejection, truncation checks, and max-codepoint  belong to `TextUtils`.
...
- `ConceptBlockGrmtCompiler.cu` receives that grouping, constructs `UniByte` for tokenization, and calls `tokenizer.unigramLM().trainFromCorpus(corpus, tokenizer_hp)` while building vocab + GRMT artifacts.
- `LoadInferenceTokenizer()` and `tokenizer_self_test.cu` construct `UniByte` from the same grouping for runtime /loading. `LoadTrainingData()` consumes the resulting `.grmt` header/runtime token-layout facts instead of constructing `UniByte`.
...
- Trie is built from learned unigram pieces only. Layout special and fixed newline tokens are not trie entries.
...
- `createAtomTableFromRawTextDetections()` finalizes detected atom spans and allocates both registries. `AtomTable` registration retains its durable `atom_entry_id` semantics. Independently, `SequenceLocalAtomTable::ticket()` assigns the exact detected value an index within its atom type and writes only that `local_atom_index` onto the span. Neither address is derived from the other.
...
- `AtomValue` and `parseAtom()` cover the five active authored atom types: integer, float, string, boolean, and entity. Exact numeric side channels remain exclusive to integer and float entries; there is no generic/unknown payload path.
...
## Sequence-local atom indexing
- `SequenceLocalAtomTable` owns one dense index space per `AtomType`; its complete address is `(type, local_index)`.
- Exact repeated values within the same type reuse their local index. Different types may simultaneously own index zero.
- `token_local_atom_indices` is populated only at typed opening boundaries. The boundary token already carries the type.
- Local ticketing reads detected type and raw value directly. It must not read, translate, or otherwise depend on `atom_entry_id`.
- Each finalized sliding-window row receives a newly compacted table. Local indices are rebased from their source local table without consulting the durable `AtomTable`.
...
- SFT supervision is field-list-driven through `training.config.supervised_fields` and `training.config.unsupervised_fields` in `ai_config.json`. The canonical renderer orders state as target state, success criteria/evidence, constraints, knowns/unknowns, reasoning/intermediates, determine, define, execute, update, then answer. Its Determine/Define/Execute/Update spans include the full labeled section and following separator; Answer is model-visible as `<answer>...</answer>` and its span includes both labels. GRMT serialization remains neutral and retains those spans. At the Phase 1 batching boundary, `applySlidingWindows()` invokes `projectConceptSupervision()` from `ConceptSupervision.cu` to unmask every present supervised field, retain listed unsupervised fields as masked context, ignore unlisted fields for loss, and truncate after the last listed field. If the final retained token is supervised, it may also predict EOS after boundary insertion.
- `ConceptBlockGrmtCompiler.cu` tokenizes raw content only. It must not add `BOS`/`EOS`; Phase 1 startup routes sequences through `SlidingWindow.cu` for that layout work.
- `UniByte` intentionally exposes per-text encode paths only. Do not reintroduce `encodeBatch()` / vector-of-vector tokenization; GRMT compilation belongs to `ConceptBlockGrmtCompiler.cu`, while `BOS`/`EOS` and sequence windows belong to the runtime `DataLoader.cu` → `SlidingWindow.cu` startup path.
- Minimum text-length gating is config-owned: `training.config.tokenizer_min_cleaned_text_length` is loaded into `TrainingHyperparameters`, sliced directly through `TokenizerHP`, and consumed by `ConceptBlockGrmtCompiler.cu` before GRMT encoding. Do not hard-code this threshold in the compiler.
