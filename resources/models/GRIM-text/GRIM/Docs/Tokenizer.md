...
- `train_tokenizer.cu` loads `AiConfigSnapshot::document`, finalizes it through HyperParameters, creates the grouping, and passes it into `PrepareTrainingDataFromCache()`.
...
Do not hand-copy raw snapshot `tokenizer_*` document leaves plus `TrainingHyperparameters` into a tokenizer wrapper or `StartupConfig::tokenizer_*` mirror. `UniByte` stores `TokenizerHP` directly; isolated tokenizer tests use explicit `TokenizerHP` fixtures when they need non-config startup values.
