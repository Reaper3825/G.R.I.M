...
- SFT supervision is driven by recursive `training.config.concept_spans` objects. Every scalar, repeated entry, and parent uses the same delimiter-inclusive span rules; child policy inherits or overrides its parent. `context` retains masked tokens, `supervised` assigns causal targets, and `ignore` removes tokens with tree/side-channel remapping. Exact delimiter IDs are resolved once after vocabulary load. GRMT v34 stores a generic named tree and structural layout identity rather than fixed fields; adding configured spans requires corpus regeneration but no further GRMT version change.
