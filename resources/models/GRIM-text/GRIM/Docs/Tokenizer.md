...
- `AtomEntry::arg_number` is durable per-entry metadata, not a parallel sidecar vector. Deduped numeric atoms must share one stored decomposition, and AtomTable `saveToFile()` / `loadFromFile()` / `saveToTextFile()` must persist that metadata with the entry.
