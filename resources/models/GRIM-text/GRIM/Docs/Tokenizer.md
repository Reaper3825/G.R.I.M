...
- Detector-emitted atom spans are an upstream contract: if a detector marks a span as `ATOM_INT` / `ATOM_FLOAT` and `AtomTable::parseAtom()` rejects the raw text, both runtime tokenization and tokenizer training must throw immediately. Do not route that span back through unigram text tokenization and do not silently skip it during training.
- `createAtomTableFromRawTextDetections()` is the runtime AtomTable creation boundary: it allocates the per-sequence `AtomTable`, converts `RawTextDetection` records into `StructuralSpan`s, registers atom-emitting spans exactly once, and returns finalized spans with `atom_entry_id` set before any unigram segmentation of non-atom gaps.
- `UniByte::tokenizeWithMetadata()` must call `createAtomTableFromRawTextDetections()` immediately after `DetectorRegistry::scan()`. The later merge loop reuses returned `StructuralSpan::atom_entry_id` values and emits the same placeholder token; do not create a parallel finalized-span record and do not call `registerSpan()` a second time during placeholder merge.
