...
- `createAtomTableFromRawTextDetections()` also owns `arg_number` population for numeric atoms. It must decompose detector-approved signed integers, decimals, leading-dot floats, and scientific notation into mantissa `DigitBinding` records plus sign, decimal, and exponent metadata. `malformed_numbers` is not a recovery path for detector-emitted numeric atoms: if the side channel cannot be populated, throw with detector name, raw text, byte offsets, atom type, and reason.
