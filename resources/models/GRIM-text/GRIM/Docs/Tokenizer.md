...
- `UniByte::tokenizeWithMetadata()` must register detector-emitted atom spans into the per-sequence `AtomTable` immediately after detection and before any unigram segmentation of non-atom gaps. The later merge loop reuses `StructuralSpan::atom_entry_id` and emits the same placeholder token; do not create a parallel finalized-span record and do not call `registerSpan()` a second time during placeholder merge.
