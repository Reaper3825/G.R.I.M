...
Phase1 handoff boundary: loss configuration, model vocab/capacity, parameter registration, class-weight storage ownership, and batch semantic construction are all decided before any loss graph node exists. Runtime loss assembly only consumes the Phase1-authored `BatchPayload`, uploaded `BatchDeviceBindings`, borrowed `LossConfigHP`, and current graph logits. `NLLLossGradFn` may own tape-local `log_softmax_result` / `grad_log_probs` buffers for the active graph window, but it must not allocate or synthesize targets, masks, MTP geometry, or loss config after the handoff.
