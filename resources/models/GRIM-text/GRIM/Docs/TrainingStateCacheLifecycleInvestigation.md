...
- `training/Phases/Phase2_InferenceLoop.cu`: inference scoring receives `BatchPayload`, calls `Batching::uploadBatchToDevice()`, and consumes only the returned `BatchDeviceBindings` for that call. Phase2 owns the evolving token sequence.
