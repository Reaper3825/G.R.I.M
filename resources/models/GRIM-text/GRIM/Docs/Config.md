...
Subprocess coordinator code follows the same single-root rule: load one `AiConfigSnapshot`, slice `SubprocessConfig` from that snapshot, and pass the resolved config path onward as needed. Do not add a second path-based `SubprocessConfig` loader that reparses `ai_config.json` inside `training/Subprocess/`.
