...
Mixed precision is configured once at startup via `training.config.precision.parameter_groups`. These authored fields live on `LanguageModelConfig` as per-parameter-group precision values, and each registered `ParameterGroup` is stamped during `ParameterGroupRegistration` with its `ParameterGroupPrecision` (`fp32` or `bf16_compute`). `LanguageModelConfig` does not own a model-wide precision mode; kernels must consume the registered group metadata explicitly.
