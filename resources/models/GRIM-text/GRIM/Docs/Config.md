...
- `control/ai_config_paths.hpp` owns the raw `AiConfigSnapshot`: one strict collapsed snapshot of authored `ai_config.json` values only. It loads the file, normalizes nested JSON into a single authored field surface, and performs presence/type checks only. It must not own derived values, grouped views, fallback defaults, or a second typed config layer. Missing authored JSON fields must fail loud here or at the immediate HyperParameters handoff — never via optional parsing.
- `Shared/HyperParameters/HyperParameters_GPU.hpp` owns the first authoritative typed config structs, the snapshot → typed assignment boundary, derivation, validation, and final durable config handoff (`TrainingHyperparameters`, `LanguageModelConfig`, `StartupConfig`, etc.).
