...
Computed startup cache capacity is owned by grouped HP views. `modelCacheCapacityHP()` validates the training `LanguageModelConfig` cache rectangle (`max_cached_batch`, `max_cached_seq_len`, `max_tokens_per_batch`). `trainingStateRuntimeCacheHP()` extends that validated rectangle with `d_model`, `vocab_size`, and logits token capacity for `TrainingState::allocateStepDeviceWorkspaces()`. Inference uses `InferenceCacheConstructionHP` for its prompt/decode capacity and converts it to the TrainingState allocation view with `trainingStateRuntimeCacheHPFromInferenceCache()`. Do not re-check or recompute the cache rectangle in server startup, diagnostics, or per-batch code; `TrainingState` may validate the grouped view it receives but must not read config or derive its own capacity.
