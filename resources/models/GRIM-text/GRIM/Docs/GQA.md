...
At the TensorContract boundary, the finalized grouped ratio rides on `HyperparameterGroupings.hpp::EncoderSelfAttentionHP::heads_per_kv_group` so SDPA/backward reduction consumes the HyperParameters-owned value instead of recomputing `num_heads / num_kv_heads` locally.
