...
Durable PBM device state is owned at model level by `LanguageModel::pbm_owner_` (`PBM::PBMStateOwner`, implemented in `Shared/PBM/PBMStateOwner.hpp/.cu`). The owner releases ALiBi slopes, RoPE inverse frequencies, and the upload event through RAII; it does **not** own static geometry, construction-policy mirrors, or the derived host tables after startup. Consumers borrow the same `PBMState` by const reference/pointer instead of a second mirror/view struct. Static geometry such as `rotary_dim`, `num_heads`, and `num_kv_heads`, plus the immutable host PBM tables, stay on config-owned grouped attention HP. `StreamController` supplies the initialization stream but does not own PBM buffers; `EmbeddingLayer` owns durable token weights; `BatchPayload` supplies per-call sequence geometry but does not own PBM state; autograd intermediates own transient Q/K/V/tape tensors only.
