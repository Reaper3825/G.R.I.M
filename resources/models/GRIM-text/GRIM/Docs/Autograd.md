...
Static loss configuration is an explicit boundary payload, not Autograd runtime state. Phase2 derives `LossConfigHP` from authoritative `TrainingContext.config.hyperparameters` only at the `autogradTrainingStep()` call boundary, which passes the required `LossConfigHP&` into `computeAutogradLoss()` and MTP loss assembly. `AutogradContext` must not store, default-construct, assign, or revalidate loss config after that boundary. Runtime loss inputs such as class-balanced device weights remain separate `TrainingState` buffers.

`autogradTrainingStep()` must not take a sidecar `LanguageModelConfig` beside `LanguageModel& model`. `LanguageModel` already owns the authoritative startup `LanguageModelConfig` and autograd reads it through `model.getConfig()`. The only extra config payload allowed at this boundary is the explicit loss grouping that the model does not own.

Phase1/Phase1Startup also completes the durable handoff long before the autograd graph exists: model config/capacity, RNG/init seed, layer construction, registered parameter groups and their persistent grad buffers, `TrainingState` step-cache tensors, and stream/cuBLAS owners are already established before Phase2 builds any per-batch graph. Phase2 then derives the explicit loss grouping from `ctx.config.hyperparameters` before entering autograd. The graph-time objects created later (`AutogradIntermediates::logits_tensor`, `LogSoftmaxGradFn` saved log-probs, `NLLLossGradFn`, CE scalar reduction scratch, per-op saved caches, and non-leaf gradient scratch) are Category 1 tape state; they must not recreate or reinterpret startup-authored semantics.

MTP shifted-target device pointers are part of the same prepared payload boundary: `BatchPayload.mtp_shifted_targets` carries the Phase1-authored host semantics, `uploadBatchToDevice()` copies those targets into `TrainingState.cached_mtp_shifted_targets_tensor`, and `BatchDeviceBindings.d_mtp_shifted_targets` is the only loss-facing device view. `BatchDeviceBindings` must not duplicate `mtp_k`; MTP geometry comes from `BatchPayload` and model config. `AutogradTraining.cu` resolves the LM-head-equivalent hidden representation, then `training/Autograd/AutogradMtpAuxiliaryLoss.cu::computeAutogradMtpAuxiliaryLosses()` consumes that explicit `mtp_input`, model-owned MTP heads, payload, bindings, and `LossConfigHP`. `Shared/MTP/MTP_GPU.cu` must remain a narrow CUDA primitive file (accuracy kernel only): it must not include or read `AutogradContext`, `AutogradIntermediates`, `TrainingState`, `LanguageModelConfig`, or `LMHeadLayer`. MTP loss assembly must not allocate per-head target tensors or perform H2D copies.

MTP alpha warmup is keyed by the monotonic Phase2 `TrainingContext::global_step`, but autograd must not carry a copied `global_step` field. Phase2 derives the per-batch `mtp_alpha_effective` scalar before entering `autogradTrainingStep()`, and MTP loss assembly consumes that scalar. `AutogradContext` only carries graph/loss runtime inputs, while `TrainingState::autograd_batch_idx` remains an epoch-local forward seed input for stochastic kernels.
