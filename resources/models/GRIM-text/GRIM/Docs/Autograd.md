...
MTP shifted-target device pointers are part of the same prepared payload boundary: `BatchPayload.mtp_shifted_targets` carries the Phase1-authored host semantics, `Batching::uploadBatchToDevice()` copies those targets into the payload-attached `BatchDeviceStorage.mtp_shifted_targets_tensor`, and `BatchDeviceBindings.d_mtp_shifted_targets` is the only loss-facing device view. `BatchDeviceBindings` must not duplicate `mtp_k`; MTP geometry comes from `BatchPayload` and config. `computeMTPAuxiliaryLosses()` must not allocate per-head target tensors or perform H2D copies inside loss assembly.

MTP alpha warmup is keyed by the monotonic Phase2 `TrainingContext::global_step`, but autograd must not carry a copied `global_step` field. Phase2 derives the per-batch `mtp_alpha_effective` scalar before entering the autograd loss boundary and uses that scalar only to decide whether the shared-forward request should ask `Forward::executeModelForward(...)` to emit MTP logits on this training batch. `computeAutogradLoss()` then consumes those precomputed shared-forward logits. Inference may request the same MTP logits through the returned `ModelForwardOutputs` without entering any training-only adapter. `AutogradContext` only carries graph/loss runtime inputs, while `TrainingState::autograd_batch_idx` remains an epoch-local forward seed input for stochastic kernels.
...
- `AutogradSelectorSupervisionLoss.cu` — final-state decode-time selector CE. It consumes Phase1-authored selector targets, stores detached selector inputs/results on the active `ModelForwardOutputs` sink returned for the current training forward call, adds CE into the caller-owned `AutogradLossState::loss_tensor`, and returns the weighted selector scalar.

Decode-time selector candidate construction follows the prepared-payload boundary too: `buildDecodeTimeCandidateSet(selector_hp, payload, bindings, exec_memory, runtime.decode_time_selector_runtime, row, stream)` consumes `BatchPayload` row semantics plus `BatchDeviceBindings` GPU upload addresses, then packs live slot ids/features from `ExecutionMemory` into the caller-owned runtime workspace on GPU. `Shared/Execution/DecodeTimeNumPolicy.{hpp,cu}` now exports free ops; it is not a device-buffer owner or durable policy object. The only host mirror is the live-slot count needed for Tensor shape construction; selector supervision maps a host-authored target slot through `resolveDecodeTimeTargetIndexForSlot(selector_hp, ..., runtime.decode_time_selector_runtime, ...)` and copies back only the scalar CE class index. Fixed slot feature vectors must not be staged through host buffers.
...
Parameter gradient zeroing is owned by the TensorContract `ParameterGroup` inventory. `executeAutogradBackward()` calls `zeroParameterGradients(model.parameterGroups(), stream)` only when `accumulate=false`; it must not enumerate embedding, LM-head, encoder, MTP, execution-block, or selector tensors directly. Adding a trainable tensor means placing its inventory entry on the startup parameter-registry boundary (`Startup/Model/ParameterRegistry.hpp` for migrated inventory slices, otherwise the current `ParameterGroupRegistration` inventory code) so optimizer stepping, clipping, diagnostics, and zeroing all see the same source of truth.
...
During gradient accumulation, existing parameter grad buffers already contain prior microbatch contributions. A post-backward nonzero/RMS check proves only that the cumulative buffer is nonzero, not that the current backward pass delivered signal. Accumulation-slot verification must snapshot expected signal tensors before `AutogradLossState::loss_tensor.backward()` and require a finite nonzero pre/post delta after backward.
