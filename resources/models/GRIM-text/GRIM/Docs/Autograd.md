...
MTP alpha warmup is keyed by the monotonic Phase2 `TrainingContext::global_step`, but autograd must not carry a copied `global_step` field. Phase2 derives the per-batch `mtp_alpha_effective` scalar before entering the autograd loss boundary and uses that scalar only to decide whether the shared-forward request should ask `Forward::executeModelForward(...)` to emit MTP logits on this training batch. `computeAutogradLoss()` then consumes those precomputed shared-forward logits. Inference may request the same MTP logits through `GenerationState::forward_outputs` without entering any training-only adapter. `AutogradContext` only carries graph/loss runtime inputs, while `TrainingState::autograd_batch_idx` remains an epoch-local forward seed input for stochastic kernels.
