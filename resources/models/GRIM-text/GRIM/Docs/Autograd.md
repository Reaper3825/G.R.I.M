...
GradFns own Category 1 saved forward data and non-leaf gradient scratch only for the active tape window. Leaf tensors reuse their persistent `Tensor.grad_` buffers; non-leaf tensors allocate owned scratch, zero it before use, and release it from `release_saved()` / RAII deleters after `Tensor::backward()` has synchronized. Large loss buffers currently follow this same immediate GradFn lifecycle: `NLLLossGradFn` owns `log_softmax_result` and its `grad_log_probs` scratch via RAII guards, releases the upstream `LogSoftmaxGradFn`, and queues GPU cleanup at the tape boundary. This is lifecycle cleanup only; promoting full-vocab loss workspaces to Phase1/TrainingState preallocation is a separate architectural change.
