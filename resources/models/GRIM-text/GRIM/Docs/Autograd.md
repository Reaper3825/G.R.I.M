...
Decode-time selector candidate construction follows the prepared-payload boundary too: `buildDecodeTimeCandidateSet(selector_hp, payload, bindings, exec_memory, runtime.decode_time_selector_runtime, row, stream)` consumes `BatchPayload` row semantics plus `BatchDeviceBindings` GPU upload addresses, then packs live slot ids/features from `ExecutionMemory` into the caller-owned runtime workspace on GPU. Inference resolves decode-time selection through the same boundary via `resolveDecodeTimeNumSlotSelectionOrMask(selector, selector_hp, payload, bindings, runtime, row, ...)`; the old raw-pointer candidate-set overload is gone. `Shared/Execution/DecodeTimeNumPolicy.{hpp,cu}` now exports free ops; it is not a device-buffer owner or durable policy object. The only host mirror is the live-slot count needed for Tensor shape construction; selector supervision maps a host-authored target slot through `resolveDecodeTimeTargetIndexForSlot(selector_hp, ..., runtime.decode_time_selector_runtime, ...)` and copies back only the scalar CE class index. Fixed slot feature vectors must not be staged through host buffers.
