...
Static loss configuration is a Phase1 handoff, not Autograd runtime state. Phase2 passes `TrainingContext.loss_config` directly into `autogradTrainingStep()`, which passes the required `LossConfigHP&` into `computeAutogradLoss()` and MTP loss assembly. `AutogradContext` must not store, default-construct, assign, or revalidate loss config after the Phase1 grouping boundary. Runtime loss inputs such as class-balanced device weights remain separate `TrainingState` buffers.

Phase1/Phase1Startup also completes the durable handoff long before the autograd graph exists: model config/capacity, RNG/init seed, layer construction, registered parameter groups and their persistent grad buffers, `TrainingState` step-cache tensors, stream/cuBLAS owners, and `TrainingContext.loss_config` are already established before Phase2 builds any per-batch graph. The graph-time objects created later (`AutogradIntermediates::logits_tensor`, `LogSoftmaxGradFn` saved log-probs, `NLLLossGradFn`, CE scalar reduction scratch, per-op saved caches, and non-leaf gradient scratch) are Category 1 tape state; they must not recreate or reinterpret Phase1-authored semantics.
