...
The explicit Phase2 training path reads configuration from the Phase1-authored `TrainingContext.config` snapshot. It must not create or pass a sidecar `LanguageModelConfig`; autograd and forward receive the authoritative snapshot or grouped views sliced directly from it. The only extra config payload allowed at the loss boundary is the explicit loss grouping.
