...
`TrainingState` owns runtime allocations only. Cache capacity is authored on `LanguageModelConfig` by Phase1 startup config construction, but the runtime owner must not accept raw config. `Startup/Model/ModelGpuAssembly.cu` owns the startup runtime bootstrap functions that verify resource order, slice `HyperParameters::TrainingStateWorkspaceHP` from the finalized root config, and pass only that explicit workspace grouping plus the primary stream to `TrainingState::allocateStepDeviceWorkspaces()`. `max_cached_seq_len` belongs to prompt/KV/payload capacity paths, not the TrainingState token-cache boundary, and root config validation stays on the caller-owned config side rather than inside the TrainingState allocator. Inference startup is not a manual CUDA/model/bootstrap path and is not owned by `grim_text_server`; `train_gpu --inference` calls `executePhase1(...INFERENCE)`, owns the returned `TrainingContext`, and exposes Phase2 inference through the internal worker endpoint.

Trainable parameter verification is not a `TrainingState` responsibility. `Startup/Model/ParameterGroupRegistration.{hpp,cu}` runs after `GRIMText::Training::Startup::assembleGpuModel(model, weight_init_seed)` in the main startup path and is the single registration-time gate for layer-owned parameter tensors: non-null data, non-null grad buffer, non-empty shape, config-disabled tensor absence, and duplicate data-alias detection. Do not add individual embedding/LM-head/encoder weight checks to the startup runtime allocator.
