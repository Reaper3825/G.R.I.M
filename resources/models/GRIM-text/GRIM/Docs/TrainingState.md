...
Phase2 owns token-by-token generation chronology. `LanguageModel` does not own a generation session, KV cursor, KV cache, or single-token decode scratch; it only scores the `BatchPayload` Phase2 passes to `getNextTokenLogits()`.
...
`Shared/Forward/ModelForwardRuntimePayload.hpp` owns the mutable runtime sink boundary for shared forward. `Forward::ModelForwardRequest` carries the immutable model/input side, while `executeModelForward(...)` receives `ModelForwardRuntimePayload` directly as a sibling payload. Training callers fill that runtime payload from `TrainingState`; inference-prefill callers fill it from `GenerationState` plus `AutogradIntermediates`. Shared forward must write only those explicit payload sinks.
