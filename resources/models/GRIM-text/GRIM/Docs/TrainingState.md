...
`Shared/Forward/ModelForwardRuntimePayload.hpp` owns the mutable runtime payload for shared forward. `Forward::ModelForwardRequest` carries the immutable model/input side, `ModelForwardRuntimePayload` carries the mutable execution-runtime/read-gate siblings, and `executeModelForward(...)` returns the per-call `ModelForwardOutputs` owner directly. Training passes `TrainingState::execution_runtime`; Phase2 inference passes `GenerationState::execution_runtime`. Shared forward must write only those explicit payload sinks and return values.

Inference session state is **not** `TrainingState`: persistent execution memory, typed execution trace runtime, and decode-time selector result are `GenerationState` fields reset by Phase2 inference. The live shared-forward sink is now the returned `ModelForwardOutputs` object for the current call window, not durable session state. Training's `execution_runtime` is training/eval forward diagnostics only.
