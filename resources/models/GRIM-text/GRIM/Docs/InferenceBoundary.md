...
- [x] Route inference prefill through the shared primitive with `ModelForwardGraphPolicy{false,false,false}`: read-only parameter graph, no dropout, and optional forward extras requested explicitly.
