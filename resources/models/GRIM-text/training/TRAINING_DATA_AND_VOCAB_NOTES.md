...
- **At training startup:** Vocab size is **not** read from `ai_config.json`, `vocab.bin`, or tokenizer internals. It is read from the **.grmt file header** when `Shared/DataLoader/DataLoader.cu` loads the GRMT corpus through `GrmtCorpusReader`. Phase 1 stores that header fact on `ctx.data.vocab_size`, syncs `ctx.config.vocab_size` from it before model allocation, and Phase2/diagnostics consume `payload.vocab_size` (or finalized `config.vocab_size` when no payload exists). `LanguageModelConfig::vocab_size` is only the model-allocation copy of that GRMT fact, never the author.
