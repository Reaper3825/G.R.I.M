...
**Check:** Log the GRMT header fact `ctx.data.vocab_size` after `LoadTrainingData` / `ModelAllocated`; `ctx.config.vocab_size` is the authoritative finalized model width.
