...
EffectiveConfig compileEffectiveConfig(const json& model_config) {
    ...
    if (c.tokenizer_unk_token != "<unk>" || c.tokenizer_pad_token != "<pad>" ||
        ...
        throw std::runtime_error("configured special-token strings do not match KTMG v9 token layout");
    }
    ...
    if (specials.size() != c.tokenizer_special_tokens.size() || specials.size() != 4 ||
        ...
        throw std::runtime_error("tokenizer_special_tokens must contain each KTMG v9 special token exactly once");
    }
