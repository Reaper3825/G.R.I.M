...
- [x] **Shared/UnigramByte/Unigram.cu** ✅ AUDITED
  ...
  - Pattern to check: Search for learned-vocab writes that bypass `VocabWriteOp.hpp` (Rule 20: no split vocab writes)
...
- [x] **Shared/UnigramByte/AhoCorasick.cu** ✅ AUDITED
  ...
  - Detection confirmed BEFORE Viterbi encoding (`tokenizeWithMetadata`: scan → filter atom detections → Viterbi segmentation) ✅
...
- [x] **Shared/Optimizers/AdamW/AdamW_Kernal_GPU.cu** ✅ AUDITED & HARDENED
  ...
  - Optimizer states (`m_states`, `v_states`) are allocated centrally in `OptimizerState::allocate()` and bound to registry-owned parameter groups after `Startup/Model/ParameterGroupRegistration::buildParameterGroups(config, gpu_model_state, parameter_registry)` ✅
...
- [x] **AutogradTraining.cu** ✅ AUDITED & FULLY REFACTORED (2070 lines, 4 major functions)
  ...
  - **PRIMARY LOSS PATH**: `computeAutogradLoss(ctx, payload, loss_config, mtp_alpha_effective)` — text CE + auxiliary loss assembly, returns `LossResult`
