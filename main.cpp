...
private:
    static void wrapLine(const WrappedLine& ln, float maxW, sf::Text& meas, std::vector<WrappedLine>& out) {
        ...
        while (iss >> word) {
            ...
            meas.setString(test);
            if (meas.getLocalBounds().width <= maxW) current=test;
            else {
                if (current.empty()) {
                    std::string accum;
                    for (char c: word) {
...
static fs::path resolvePath(const fs::path& cur,const std::string& u){
    std::error_code ec; fs::path p(u); return p.is_absolute()?fs::weakly_canonical(p,ec):fs::weakly_canonical(cur/p,ec);
}
// -------------------------------------------
...
int main(int argc, char** argv) {
    ...
    NLP nlp; {
        ...
        fs::path rulesExe=exeDir/"nlp_rules.json"; fs::path rulesCwd="nlp_rules.json"; std::string err;
    ...
    sf::RenderWindow window(sf::VideoMode(500,800),"GRIM"); window.setVerticalSyncEnabled(true);

    sf::Font font; std::string fontPath=findAnyFontInResources(argc,argv);
    if(fontPath.empty()||!font.loadFromFile(fontPath)) std::cerr<<"[WARN] Could not load a font. Put a .ttf in resources/.\n";
    else std::cerr<<"[INFO] Using font: "<<fontPath<<"\n";

    sf::RectangleShape titleBar; titleBar.setFillColor(sf::Color(26,26,30));
    sf::Text titleText; if(font.getInfo().family!=""){ titleText.setFont(font); titleText.setCharacterSize(kTitleFontSize); titleText.setFillColor(sf::Color(220,220,235)); titleText.setString("G R I M"); }
    sf::RectangleShape inputBar; inputBar.setFillColor(sf::Color(30,30,35));
    sf::Text inputText,lineText; if(font.getInfo().family!=""){ inputText.setFont(font); inputText.setCharacterSize(kFontSize); inputText.setFillColor(sf::Color::White); lineText.setFont(font); lineText.setCharacterSize(kFontSize); lineText.setFillColor(sf::Color::White); }

    ConsoleHistory history; auto addHistory=[&](const std::string&s,sf::Color c=sf::Color::White){ history.push(s,c); std::cout<<s<<"\n"; };
    std::string buffer; fs::path currentDir=fs::current_path(); addHistory("GRIM is ready. Type 'help' for commands. Type 'quit' to exit.",sf::Color(160,200,255));

    sf::Clock caretClock; bool caretVisible=true; float scrollOffsetLines=0.f;
    auto clampScroll=[&](float m){ if(scrollOffsetLines<0)scrollOffsetLines=0; if(scrollOffsetLines>m)scrollOffsetLines=m; };
    ...
    while(window.isOpen()){
        ...
        while(window.pollEvent(e)){
            ...
            if(e.type==sf::Event::MouseWheelScrolled && e.mouseWheelScroll.wheel==sf::Mouse::VerticalWheel){
                scrollOffsetLines+=(e.mouseWheelScroll.delta>0?3.f:-3.f); if(scrollOffsetLines<0)scrollOffsetLines=0.f;
            }
            if(e.type==sf::Event::TextEntered){
                if(e.text.unicode==8){ if(!buffer.empty()) buffer.pop_back(); }
                    ...
                    if(cmd=="help"){ addHistory("Commands:",sf::Color(255,210,120));
                        addHistory("  help - show this help"); addHistory("  pwd - print current directory");
                        addHistory("  cd <path> - change directory"); addHistory("  list - list current directory");
                        addHistory("  mkdir <path> - create directory"); addHistory("  rm <path> - remove file/dir");
                        addHistory("  reloadnlp - reload nlp_rules.json"); addHistory("  grim <prompt> - ask local model");
                        scrollOffsetLines=0.f; continue; }

                    if(cmd=="pwd"){ addHistory(currentDir.string()); scrollOffsetLines=0.f; continue; }
                    if(cmd=="cd"){ if(args.size()<2){ addHistory("Error: cd requires path."); continue; }
                        fs::path t=resolvePath(currentDir,args[1]); std::error_code ec;
                        if(!fs::exists(t,ec)) addHistory("Error: path does not exist.");
                        else if(!fs::is_directory(t,ec)) addHistory("Error: not a directory.");
                        else{ currentDir=t; addHistory("Directory changed to: "+currentDir.string()); }
                        scrollOffsetLines=0.f; continue; }

                    if(cmd=="list"){ std::error_code ec;
                        if(!fs::exists(currentDir,ec)||!fs::is_directory(currentDir,ec)){ addHistory("Error: invalid dir."); continue; }
                        for(auto& ent: fs::directory_iterator(currentDir,ec)){ if(ec) break; bool d=ent.is_directory(ec);
                            addHistory(std::string(d?"[D] ":"    ")+ent.path().filename().string()); }
                        scrollOffsetLines=0.f; continue; }

                    if(cmd=="mkdir"){ if(args.size()<2){ addHistory("Error: mkdir requires path."); continue; }
                        fs::path p=resolvePath(currentDir,args[1]); std::error_code ec; fs::create_directories(p,ec);
                        if(ec) addHistory("Error creating dir: "+ec.message()); else addHistory("Directory created.");
                        scrollOffsetLines=0.f; continue; }

                    if(cmd=="rm"){ if(args.size()<2){ addHistory("Error: rm requires path."); continue; }
                        fs::path p=resolvePath(currentDir,args[1]); std::error_code ec;
                        if(!fs::exists(p,ec)) addHistory("Error: path does not exist.");
                        else if(!fs::remove(p,ec)) addHistory("Error: could not remove (not empty?).");
                    ...
                    if(cmd=="grim"){ std::string q=(line.size()>4)?trim(line.substr(4)):"";
                        if(q.empty()){ addHistory("Usage: grim <your question>"); continue; }
                        try{ std::string ans=callAI(q); if(ans.empty()) ans="[grim] (empty)";
                            std::istringstream iss(ans); std::string l; while(std::getline(iss,l)) addHistory(l,sf::Color(200,220,255));
                        } catch(const std::exception&ex){ addHistory(std::string("[grim] Error: ")+ex.what(),sf::Color(255,140,140)); }
                        scrollOffsetLines=0.f; continue; }
                    ...
                    if(!intent.matched) addHistory("[NLP] No intent matched.",sf::Color(255,200,140));
                        ...
                        if(intent.name=="open_app"){ auto it=intent.slots.find("app"); if(it!=intent.slots.end()) addHistory("Would open app: "+it->second); }
                        else if(intent.name=="search_web"){ auto it=intent.slots.find("query"); if(it!=intent.slots.end()) addHistory("Would search web for: "+it->second); }
                        else if(intent.name=="set_timer"){ auto it=intent.slots.find("minutes"); if(it!=intent.slots.end()) addHistory("Would set timer for "+it->second+" minute(s)."); }
                    }
        ...
        // Layout
        sf::Vector2u ws=window.getSize(); float winW=ws.x, winH=ws.y;
        ...
        if(font.getInfo().family!=""){ sf::FloatRect tb=titleText.getLocalBounds();
            titleText.setPosition((winW-tb.width)*0.5f,(kTitleBarH-tb.height)*0.5f-6.f); }
        if(font.getInfo().family!=""){ std::string toShow=buffer; if(caretVisible) toShow.push_back('_');
            inputText.setString(toShow); inputText.setPosition(kSidePad,winH-kInputBarH+(kInputBarH-(float)kFontSize)*0.5f-2.f); }
        ...
        if(font.getInfo().family!=""){
            ...
            float histH=std::max(0.f,histBottom-histTop), wrapW=std::max(10.f,winW-2.f*kSidePad);
            history.ensureWrapped(wrapW,lineText);
            float viewLines=std::max(1.f,histH/lineH); size_t wrapCount=history.wrappedCount();
            float maxScroll=(wrapCount>(size_t)viewLines)?(wrapCount-(size_t)viewLines):0.f; clampScroll(maxScroll);

            window.clear(sf::Color(18,18,22)); window.draw(titleBar); window.draw(titleText);
            ...
            float y=histTop;
            for(long i=start;i<end;++i){ if(i<0||i>=(long)history.wrapped().size()) continue;
                auto& wl=history.wrapped()[i]; lineText.setString(wl.text); lineText.setFillColor(wl.color);
                lineText.setPosition(kSidePad,y); window.draw(lineText); y+=lineH; if(y>histBottom) break; }

            if(wrapCount>(size_t)viewLines){ float trackTop=histTop, trackH=histH;
                ...
                sf::RectangleShape track({4.f,trackH}); track.setFillColor(sf::Color(50,50,58)); track.setPosition(winW-6.f,trackTop);
            ...
            window.draw(inputBar); window.draw(inputText);
        } else { window.clear(sf::Color(18,18,22)); window.draw(titleBar); window.draw(inputBar); }
